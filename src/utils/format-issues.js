  ...
  issues.map(({ id, key, fields }) => {
    ...
    return {
      ...
      get icon() {
        ...
        const iconId =
          fields.issuetype.avatarId || iconUrl.split('/').pop().split('?')[0] || 'default';
          ...
          .then((res) => {
            try {
              ...
              if (
                ...
                // Not SVG, save as PNG
                console.error(
                  `[Icon] Got PNG/other format for ${key} (content-type: ${contentType})`,
                );
                ...
                axios
                  ...
                  .catch((err) => {
                    console.error(
                      `[Icon] Failed to re-download as binary for ${key}:`,
                      err.message,
                    );
                  });
              }
      ...
      get match() {
        let projectSplit = key.split('-');
        const project = projectSplit.shift();
        const number = projectSplit.shift();
